...
function base64_decode2(encoded) {
  ...
  if ($ === 0) {
    ...
    let n = $;
    _block = append2(encoded, repeat("=", 4 - n));
  }
  let padded = _block;
...
function replace2(result, value) {
  if (result instanceof Ok) {
    return new Ok(value);
  } else {
    return result;
    ...
    if (old instanceof Empty) {
      ...
      if (prev.key !== next.key) {
        ...
        if (prev_does_exist) {
          ...
          let change = replace3(node_index - moved_offset, next);
        ...
        if ($ instanceof Fragment) {
          ...
          if ($1 instanceof Fragment) {
            ...
            let next2 = $1;
            let new_remaining = new$5.tail;
            let change = replace3(node_index - moved_offset, next2);
          ...
          if ($1 instanceof Element) {
            ...
            if (prev2.namespace === next2.namespace && prev2.tag === next2.tag) {
              ...
              let next3 = $1;
              let new_remaining = new$5.tail;
              let change = replace3(node_index - moved_offset, next3);
...
function replace4(path, query, fragment3) {
  return from2((_) => {
    return guard(!is_browser(), undefined, () => {
      return do_replace(new Uri(relative.scheme, relative.userinfo, relative.host, relative.port, path, then$(query, non_empty), then$(fragment3, non_empty)));
    });
  });
}
...
// build/dev/javascript/ui/ffi/local_storage.ffi.mjs
...
class Authenticated extends CustomType {
  constructor(email) {
    super();
    this.email = email;
  }
}
class Unauthenticated extends CustomType {
}
class GotToken extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class SignOut extends CustomType {
}
function init2() {
  let effect = from2((dispatch2) => {
    return dispatch2(new GotToken(getItem("pie_safe_token")));
  });
  return [new Unauthenticated, effect];
}
function view2(model) {
  if (model instanceof Authenticated) {
    let email = model.email;
    return div(toList([
      class$("min-h-screen flex items-center justify-center bg-gray-50")
    ]), toList([
      div(toList([
        class$("bg-white rounded-lg shadow-md p-8 max-w-md w-full mx-4 text-center")
      ]), toList([
        h1(toList([
          class$("text-2xl font-bold text-gray-900 mb-4")
        ]), toList([text2("Welcome to Pie Safe")])),
        p(toList([class$("text-gray-600 mb-6")]), toList([text2(email)])),
        button(toList([
          class$("bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2 px-4 rounded-lg transition-colors"),
          on_click(new SignOut)
        ]), toList([text2("Sign out")]))
      ]))
    ]));
  } else {
...
function decode_email(token) {
  let parts = split2(token, ".");
  return try$((() => {
    let _pipe = drop(parts, 1);
    return first(_pipe);
  })(), (payload) => {
    return try$(base64_url_decode(payload), (bytes) => {
      return try$(bit_array_to_string(bytes), (json_string) => {
        let email_decoder = field("email", string2, (email) => {
          return success(email);
        });
        return map_error(parse2(json_string, email_decoder), (_) => {
          return;
        });
      });
    });
...
function update2(model, msg) {
  if (msg instanceof GotToken) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let token = $[0];
      let $1 = decode_email(token);
      if ($1 instanceof Ok) {
        let email = $1[0];
        return [new Authenticated(email), none()];
      } else {
        return [
          new Unauthenticated,
          replace4("/sign-in", new None, new None)
        ];
      }
    } else {
      return [
...
class InternalServerError extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class OtherError extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
class Unauthorized extends CustomType {
}
...
function do_send(req, expect, dispatch2) {
  ...
  let _pipe$2 = map_promise(_pipe$1, (response) => {
...
function post(url, body, expect) {
  return from2((dispatch2) => {
    ...
    if ($ instanceof Ok) {
      ...
      return do_send(_pipe$3, expect, dispatch2);
    } else {
...
function expect_anything(to_msg) {
  return new ExpectTextResponse((response) => {
    let _pipe = response;
    let _pipe$1 = try$(_pipe, response_to_result);
    let _pipe$2 = replace2(_pipe$1, undefined);
    return to_msg(_pipe$2);
  });
}

// build/dev/javascript/ui/pages/sign_in.mjs
...
class FormError extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class Model extends CustomType {
  constructor(email, state) {
    super();
    this.email = email;
    this.state = state;
  }
}
class UpdateEmail extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class ApiResponse extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init3() {
  return new Model("", new Idle);
}
function update3(model, msg) {
  if (msg instanceof UpdateEmail) {
    let value2 = msg[0];
    return [new Model(value2, model.state), none()];
  } else if (msg instanceof Submit) {
    let body = object2(toList([["email", string3(model.email)]]));
    return [
      new Model(model.email, new Loading),
      post("/api/auth/magic-link", body, expect_anything((var0) => {
        return new ApiResponse(var0);
      }))
    ];
  } else {
    let $ = msg[0];
    if ($ instanceof Ok) {
      return [new Model(model.email, new Success), none()];
    } else {
      let err = $[0];
      let _block;
      if (err instanceof BadUrl) {
        ...
        let body = err[0];
        _block = "Server error: " + body;
      } else if (err instanceof JsonError) {
        _block = "Unexpected response format.";
      } else if (err instanceof NetworkError2) {
        _block = "Network error. Please check your connection.";
      } else if (err instanceof NotFound) {
        _block = "Service not found.";
      ...
      let message2 = _block;
      return [new Model(model.email, new FormError(message2)), none()];
    }
  }
}
function success_view() {
  return div(toList([
    ...
    div(toList([
      ...
      h1(toList([class$("text-2xl font-bold text-gray-900 mb-4")]), toList([text2("Check your email")])),
      p(toList([class$("text-gray-600")]), toList([
        text2("We've sent you a magic link. Please check your inbox to sign in.")
      ]))
    ]))
  ]));
}
function form_view(model) {
  let _block;
  let $ = model.state;
  if ($ instanceof FormError) {
    let msg = $[0];
    _block = p(toList([class$("mt-4 text-sm text-rose-600")]), toList([text2(msg)]));
  } else {
    _block = text2("");
  }
  let error_el = _block;
  let is_loading = model.state instanceof Loading;
  return div(toList([
    class$("min-h-screen flex items-center justify-center bg-gray-50")
  ]), toList([
    div(toList([
      class$("bg-white rounded-lg shadow-md p-8 max-w-md w-full mx-4")
    ]), toList([
      h1(toList([class$("text-2xl font-bold text-gray-900 mb-6")]), toList([text2("Sign in to Pie Safe")])),
      div(toList([]), toList([
        label(toList([
        ...
        input(toList([
          type_("email"),
          value(model.email),
          class$("w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"),
          on_input((var0) => {
            return new UpdateEmail(var0);
          })
        ]))
      ])),
      error_el,
      button(toList([
        class$("mt-6 w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-2 px-4 rounded-lg transition-colors"),
        disabled(is_loading),
        on_click(new Submit)
      ]), toList([
        text2((() => {
      ...
      p(toList([class$("mt-4 text-center text-sm text-gray-600")]), toList([
...
function view3(model) {
  ...
  if ($ instanceof Success) {
    ...
    return form_view(model);
  }
}

// build/dev/javascript/ui/pages/sign_up.mjs
class Idle2 extends CustomType {
}
class Loading2 extends CustomType {
}
class Success2 extends CustomType {
}
class FormError2 extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class Model2 extends CustomType {
  constructor(first_name, last_name, family_name, email, state) {
    super();
...
class UpdateFirstName extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UpdateLastName extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UpdateFamilyName extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UpdateEmail2 extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class Submit2 extends CustomType {
}
class ApiResponse2 extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init4() {
  return new Model2("", "", "", "", new Idle2);
}
function update4(model, msg) {
  if (msg instanceof UpdateFirstName) {
    let value2 = msg[0];
    return [
      new Model2(value2, model.last_name, model.family_name, model.email, model.state),
      none()
    ];
  } else if (msg instanceof UpdateLastName) {
    let value2 = msg[0];
    return [
      new Model2(model.first_name, value2, model.family_name, model.email, model.state),
      none()
    ];
  } else if (msg instanceof UpdateFamilyName) {
    let value2 = msg[0];
    return [
      new Model2(model.first_name, model.last_name, value2, model.email, model.state),
      none()
    ];
  } else if (msg instanceof UpdateEmail2) {
    let value2 = msg[0];
    return [
      new Model2(model.first_name, model.last_name, model.family_name, value2, model.state),
      none()
    ];
  } else if (msg instanceof Submit2) {
    let body = object2(toList([
      ["first_name", string3(model.first_name)],
      ["last_name", string3(model.last_name)],
      ["family_name", string3(model.family_name)],
      ["email", string3(model.email)]
    ]));
    return [
      new Model2(model.first_name, model.last_name, model.family_name, model.email, new Loading2),
      post("/api/auth/register", body, expect_anything((var0) => {
        return new ApiResponse2(var0);
      }))
    ];
  } else {
    let $ = msg[0];
    if ($ instanceof Ok) {
      return [
        new Model2(model.first_name, model.last_name, model.family_name, model.email, new Success2),
        none()
      ];
    } else {
      let err = $[0];
      ...
      if (err instanceof BadUrl) {
        ...
        let code = err[0];
        let body = err[1];
        _block = "Error " + to_string(code) + ": " + body;
      } else {
        _block = "Unauthorized.";
      }
      let message2 = _block;
      return [
        new Model2(model.first_name, model.last_name, model.family_name, model.email, new FormError2(message2)),
        none()
      ];
    }
  }
}
function success_view2() {
  return div(toList([
    class$("min-h-screen flex items-center justify-center bg-gray-50")
  ]), toList([
    div(toList([
      ...
      h1(toList([class$("text-2xl font-bold text-gray-900 mb-4")]), toList([text2("Check your email")])),
      p(toList([class$("text-gray-600")]), toList([
        text2("We've sent you a confirmation email. Please check your inbox to complete your registration.")
      ]))
    ]))
  ]));
}
function labeled_input(label2, input_type, value2, on_input2) {
  return div(toList([]), toList([
    label(toList([
      class$("block text-sm font-medium text-gray-700 mb-1")
    ]), toList([text2(label2)])),
...
function form_view2(model) {
  let _block;
  let $ = model.state;
  if ($ instanceof FormError2) {
    let msg = $[0];
    _block = p(toList([class$("mt-4 text-sm text-rose-600")]), toList([text2(msg)]));
  } else {
    _block = text2("");
  ...
  let error_el = _block;
  let is_loading = model.state instanceof Loading2;
  return div(toList([
    class$("min-h-screen flex items-center justify-center bg-gray-50")
  ]), toList([
    div(toList([
      ...
      h1(toList([class$("text-2xl font-bold text-gray-900 mb-6")]), toList([text2("Create your account")])),
      div(toList([class$("space-y-4")]), toList([
        labeled_input("First name", "text", model.first_name, (var0) => {
          return new UpdateFirstName(var0);
        }),
        labeled_input("Last name", "text", model.last_name, (var0) => {
          return new UpdateLastName(var0);
        }),
        labeled_input("Family name", "text", model.family_name, (var0) => {
          return new UpdateFamilyName(var0);
        }),
        labeled_input("Email", "email", model.email, (var0) => {
          return new UpdateEmail2(var0);
        })
      ])),
      error_el,
      button(toList([
...
function view4(model) {
  let $ = model.state;
  if ($ instanceof Success2) {
    return success_view2();
  } else {
    return form_view2(model);
  }
}

// build/dev/javascript/ui/ui.mjs
var FILEPATH = "src/ui.gleam";

class SignUp extends CustomType {
}
class SignIn extends CustomType {
}
class Home extends CustomType {
}
class Model3 extends CustomType {
  constructor(route, sign_up, sign_in, home) {
    super();
    this.route = route;
...
class SignUpMsg extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class SignInMsg extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class HomeMsg extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
function parse_route(current_uri) {
  let $ = path_segments(current_uri.path);
  if ($ instanceof Empty) {
    return new SignIn;
  } else {
    let $1 = $.tail;
...
function on_url_change(current_uri) {
  return new OnRouteChange(parse_route(current_uri));
}
function init5(_) {
  let _block;
  let $ = do_initial_uri();
  if ($ instanceof Ok) {
    let current_uri = $[0];
    _block = parse_route(current_uri);
  } else {
    _block = new SignIn;
  }
  let initial_route = _block;
  let $1 = init2();
  let home_model;
  let home_effect;
  home_model = $1[0];
  home_effect = $1[1];
  let model = new Model3(initial_route, init4(), init3(), home_model);
  let effects = batch(toList([
    init(on_url_change),
...
function update5(model, msg) {
  if (msg instanceof OnRouteChange) {
    ...
    if (route instanceof Home) {
      ...
      home_effect = $[1];
      return [
        new Model3(route, model.sign_up, model.sign_in, home_model),
        map4(home_effect, (var0) => {
          return new HomeMsg(var0);
        })
      ];
      ...
      return [
        new Model3(route, model.sign_up, model.sign_in, model.home),
        none()
      ];
    }
  } else if (msg instanceof SignUpMsg) {
    let sub_msg = msg[0];
    let $ = update4(model.sign_up, sub_msg);
    let sub_model;
    let sub_effect;
    sub_model = $[0];
    sub_effect = $[1];
    return [
      new Model3(model.route, sub_model, model.sign_in, model.home),
      map4(sub_effect, (var0) => {
        return new SignUpMsg(var0);
      })
    ];
  } else if (msg instanceof SignInMsg) {
    let sub_msg = msg[0];
    let $ = update3(model.sign_in, sub_msg);
    let sub_model;
    let sub_effect;
    sub_model = $[0];
    sub_effect = $[1];
    return [
      new Model3(model.route, model.sign_up, sub_model, model.home),
      map4(sub_effect, (var0) => {
        return new SignInMsg(var0);
      })
    ];
  } else {
    let sub_msg = msg[0];
    let $ = update2(model.home, sub_msg);
    let sub_model;
    let sub_effect;
    sub_model = $[0];
    sub_effect = $[1];
    return [
      new Model3(model.route, model.sign_up, model.sign_in, sub_model),
      map4(sub_effect, (var0) => {
        return new HomeMsg(var0);
      })
    ];
...
function view5(model) {
  return div(toList([class$("min-h-screen bg-gray-50")]), toList([
    (() => {
      let $ = model.route;
      if ($ instanceof SignUp) {
        return map6(view4(model.sign_up), (var0) => {
          return new SignUpMsg(var0);
        });
      } else if ($ instanceof SignIn) {
        return map6(view3(model.sign_in), (var0) => {
          return new SignInMsg(var0);
        });
      } else {
        return map6(view2(model.home), (var0) => {
          return new HomeMsg(var0);
        });
      }
    })()
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "ui", 109, "main", "Pattern match failed, no pattern matched the value.", {
      value: $,
      start: 2679,
      end: 2728,
      pattern_start: 2690,
